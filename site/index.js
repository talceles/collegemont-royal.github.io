...
function GenerateHTMLCell(title, subtitle, image, i, cellClass) {
    ...
    if (cellClass.indexOf("webView") > 0) {
        return `<cell id = ${i} class="${cellClass}">${imageCode}<description><cell-title>${title}</cell-title><cell-subtitle>${subtitle}</cell-subtitle></description><iframe src=${cells[i].link}></iframe><button onclick="window.open(${cells[i].link})">Ouvrir ➜</button></cell>`
    } else {
